function getQueensMove(obj) {
	
	console.log("sqsAttackedByQ:",obj.sqsAttackedByQ);
	...
	var queenObjsArr = getSqsAttackedByQForPossibleMove(qObj);

	//find if Queen can reduce king's area 
	var tempObject = findQueensMove(queenObjsArr);

	//if findQueensMove doesn't reduce king's area, move wK
	if(tempObject.kingArea >= obj.kingArea){
		//move wK towards bK
		console.log("move the king!");
		obj.whiteKingsMoves = tempObject.whiteKingSquares
		console.log(moveWhiteKing(obj));
	}
...
function getSqsAttackedByQForPossibleMove(qObj) {
